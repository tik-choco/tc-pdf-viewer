...
export function useSync({
  ...
  const lastRemoteFromRef = useRef(null);
  ...
  const handleIncomingMessage = useCallback(
    (fromId, payload) => {
      ...
      if (parsed.type === 'accept-settings') {
        const isTarget = parsed.to == null || parsed.to === deviceId;
        if (isTarget) {
          sendCurrentSnapshot();
      ...
      lastRemoteFromRef.current = parsed.from;
  ...
  const stopCurrentConnection = useCallback(() => {
    ...
    lastRemoteFromRef.current = null;
    setHasRemoteStateDiff(false);
