// Syncs the mistlib JS wrapper (wrappers/web) into src/lib/mistlib.
//
...
// vite.config.js aliases the package to it. See .env.example.
//
// Adapted from tc-home's scripts/fetch-mistlib.mjs for this app's flat
// src/lib/mistlib layout (no nested wrappers/web subdirectory, so existing
// '../lib/mistlib/index.js' imports across the app don't need to change).
...
const wrapperDir = path.join(rootDir, "src", "lib", "mistlib");
...
// straight into wrapperDir would leave the committed tree half-replaced if
// anything below failed. Note wrapperDir (src/lib/mistlib) also holds this
// app's own gitignored node_modules-free vendor state, but only the
// WRAPPER_FILES below are ever staged/replaced.
...
for (const name of WRAPPER_FILES) {
  renameSync(path.join(stageDir, name), path.join(wrapperDir, name));
}
rmSync(stageDir, { recursive: true, force: true });

writeFileSync(revMarkerPath, resolvedSha + "\n");
console.log(`mistlib wrapper (${ref} @ ${resolvedSha.slice(0, 7)}) vendored into src/lib/mistlib`);
